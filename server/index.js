...
const TURNSTILE_SECRET_KEY = process.env.TURNSTILE_SECRET_KEY || '0x4AAAAAAEz-kGPhXL4p_ipKVs3s6g9WYuc';
  ...
  try {
    ...
    if (turnstileToken) {
      ...
      if (!isHuman) {
        ...
        return res.status(400).json({ error: 'Cloudflare verification failed.' });
      }
    }
